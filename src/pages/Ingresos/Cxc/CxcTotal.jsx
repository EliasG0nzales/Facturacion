...
const styles = `
  ...
  .tabla-cxc { width:100%; border-collapse:collapse; font-size:12px; border:1px solid #dee2e6; }
  .tabla-cxc thead tr { background-color:#00A3E1; }
  .tabla-cxc thead th { padding:6px 8px; text-align:center; font-weight:bold; color:#fff !important; white-space:nowrap; border:1px solid #dee2e6; }
  .tabla-cxc tbody tr { background:#fff; }
  .tabla-cxc tbody td { padding:5px 8px; border:1px solid #dee2e6; color:#212529; }
  .tabla-cxc .fila-total { background:#ccc !important; }
  .tabla-cxc .fila-total td { font-weight:bold; padding:8px; border:1px solid #999; }
...
const DATOS_INICIALES = [
  { ruc: '20523520025', cliente: 'Inteligente S.a.c.', docventa: 'F:FI01-000001', fecdoc: '26/02/2026', fecven: '28/02/2026', ref: '', mo: 'MN', m: 'S/.', credito: 80, saldo: 80 },
  { ruc: '', cliente: 'venta falabella', docventa: 'NV:001-000004', fecdoc: '23/01/2024', fecven: '24/01/2024', ref: '', mo: 'MN', m: 'S/.', credito: 320, saldo: 320 },
];
...
const CxcTotal = () => {
  ...
  const totalCredito = datos.reduce((a, d) => a + d.credito, 0).toFixed(2);
  ...
  const handleExcel = () => {
    const enc = ['RUC', 'CLIENTE', 'DOC.VENTA', 'FEC.DOC.', 'FEC.VEN.', 'REF.', 'MO', 'M.', 'CREDITO', 'SALDO'];
    const filas = datos.map((d) =>
      [d.ruc, `"${(d.cliente || '').replace(/"/g, '""')}"`, d.docventa, d.fecdoc, d.fecven, d.ref, d.mo, d.m, d.credito, d.saldo].join(',')
    );
    const csv = [enc.join(','), ...filas, `TOTAL,,,,,,,${totalCredito},${totalSaldo}`].join('\r\n');
    const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `cuenta_por_cobrar_pendiente_total_${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    <>
      ...
      <div className="page-container">
        <div className="page-title">
          ...
          Cuenta por Cobrar - Pendiente Total
        </div>
        ...
        <div className="reporte-titulo">CUENTA POR COBRAR - PENDIENTE TOTALIZADO</div>
